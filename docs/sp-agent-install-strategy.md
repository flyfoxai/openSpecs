...
2. 按用户选择的 agent，把对应的命令入口或 skill 一起装进去
...
这里的 `$sp-*` 是 Codex 的调用形式，本质上仍对应同一组 `sp.*` 步骤。
...
- `--ai-skills` 作为兼容别名保留
- 但 `--ai codex` 本身就足以触发 Codex skill 安装
...
- 步骤语义仍然是 `sp.*`
- 触发形式使用 `$sp-*`
- 把 skill 目录写进该 agent 的 skills 根目录
