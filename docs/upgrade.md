...
Once you've run `specify init`, the slash commands (like `/sp.specify` on dot-command hosts or `/sp-specify` on skills hosts) are **permanently installed** in your project's agent folder (`.claude/`, `.github/prompts/`, `.pi/prompts/`, etc.). Your AI coding agent reads these command files directly—no need to run `specify` again.
...
- **Test new slash commands:** Run `/sp.constitution` or `/sp-constitution` depending on your host to verify everything works
