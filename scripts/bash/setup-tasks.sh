...
if [[ ! -f "$IMPL_PLAN" ]]; then
    ...
    echo "Run the sp.plan command first to create the implementation plan (skills hosts expose this as sp-plan)." >&2
    exit 1
fi

if [[ ! -f "$FEATURE_SPEC" ]]; then
    echo "ERROR: spec.md not found in $FEATURE_DIR" >&2
    echo "Run the sp.specify command first to create the feature structure (skills hosts expose this as sp-specify)." >&2
    exit 1
fi
