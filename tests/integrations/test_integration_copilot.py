...
def command_stems() -> list[str]:
    return sorted(p.stem for p in (REPO_ROOT / "templates" / "commands").glob("*.md"))


def project_template_files() -> list[str]:
    root = REPO_ROOT / "templates" / "project"
    return sorted(p.relative_to(root).as_posix() for p in root.rglob("*") if p.is_file())


def specify_script_files(script: str) -> list[str]:
    source_dir = REPO_ROOT / "scripts" / ("bash" if script == "sh" else "powershell")
    target_dir = ".specify/scripts/bash" if script == "sh" else ".specify/scripts/powershell"
    return sorted(f"{target_dir}/{p.name}" for p in source_dir.glob("*") if p.is_file())


def specify_template_files() -> list[str]:
    root = REPO_ROOT / "templates"
    return sorted(
        f".specify/templates/{p.name}"
        for p in root.glob("*")
        if p.is_file() and p.name != "vscode-settings.json"
    )


def shared_init_files(script: str) -> list[str]:
    return sorted(
        [
            ...
            *specify_script_files(script),
            *specify_template_files(),
...
class TestCopilotIntegration:
    ...
    def test_directory_structure(self, tmp_path):
        ...
        expected_commands = {
            "analyze", "bundle", "checklist", "clarify", "constitution",
            "flow", "gate", "implement", "plan", "specify", "tasks",
            "taskstoissues", "ui",
        }
    ...
    def test_plan_references_correct_context_file(self, tmp_path):
        ...
        m = IntegrationManifest("copilot", tmp_path)
        copilot.setup(tmp_path, m)
        plan_file = tmp_path / ".github" / "agents" / "sp.plan.agent.md"
    ...
    def test_complete_file_inventory_sh(self, tmp_path):
        ...
        expected = sorted([
            *[f".github/agents/sp.{cmd}.agent.md" for cmd in command_stems()],
            *[f".github/prompts/sp.{cmd}.prompt.md" for cmd in command_stems()],
            ".github/copilot-instructions.md",
            ".specify/integrations/copilot.manifest.json",
            ".vscode/settings.json",
            *shared_init_files("sh"),
        ])
...
class TestCopilotSkillsMode:
    ...
    def test_post_process_skill_content_injects_mode(self):
        ...
        assert "mode: sp.plan" in updated
    ...
    def test_skills_have_mode_in_frontmatter(self, tmp_path):
        ...
        for f in skill_files:
            ...
            # mode should be sp.<stem>
            skill_dir_name = f.parent.name
            stem = skill_dir_name.removeprefix("sp-")
            assert fm["mode"] == f"sp.{stem}"
    ...
    def test_skills_command_refs_use_hyphen(self, tmp_path):
        """Copilot skills mode must use /sp-<name> not /sp.<name>."""
        copilot = self._make_copilot()
        created, _ = self._setup_skills(copilot, tmp_path)
        skill_files = [f for f in created if f.name == "SKILL.md"]
        assert len(skill_files) > 0
        for f in skill_files:
            ...
            assert "/sp." not in content, (
                f"{f.name} contains dot-notation /sp. reference; "
                f"skills mode must use /sp-<name>"
            )
    ...
    def test_plan_references_correct_context_file(self, tmp_path):
        ...
        copilot = self._make_copilot()
        self._setup_skills(copilot, tmp_path)
        plan_file = tmp_path / ".github" / "skills" / "sp-plan" / "SKILL.md"
        assert plan_file.exists()
        content = plan_file.read_text(encoding="utf-8")
        assert copilot.context_file in content
        assert "__CONTEXT_FILE__" not in content
    ...
    def test_build_command_invocation_skills_mode(self):
        ...
        assert copilot.build_command_invocation("sp.plan") == "/sp-plan"
        assert copilot.build_command_invocation("plan") == "/sp-plan"
        assert copilot.build_command_invocation("plan", "my args") == "/sp-plan my args"
    ...
    def test_init_with_integration_options_skills(self, tmp_path):
        ...
        assert plan_skill.exists(), "sp-plan/SKILL.md not found"
    ...
    def test_complete_file_inventory_skills_sh(self, tmp_path):
        ...
        expected = sorted([
            # Skill files
            *[f".github/skills/sp-{cmd}/SKILL.md" for cmd in command_stems()],
            # Context file
            ".github/copilot-instructions.md",
            ".specify/integrations/copilot.manifest.json",
            *shared_init_files("sh"),
    ...
    def test_dispatch_ignores_unrelated_skills_directory(self, tmp_path):
        ...
        with mock.patch("subprocess.run") as mock_run:
            ...
            assert "sp.plan" in call_args

    def test_dispatch_detects_speckit_skills_layout(self, tmp_path):
        """dispatch_command() detects sp-*/SKILL.md as skills mode."""
        copilot = self._make_copilot()
        skill_dir = tmp_path / ".github" / "skills" / "sp-plan"
        ...
        with mock.patch("subprocess.run") as mock_run:
            ...
            assert "/sp-plan" in prompt, (
                f"Skills mode prompt should invoke /sp-plan, got: {prompt}"
            )
    ...
    def test_init_skills_next_steps_show_skill_syntax(self, tmp_path):
        """specify init --integration copilot --integration-options='--skills' shows /sp-plan not /sp.plan."""
        from typer.testing import CliRunner
        from specify_cli import app
        ...
        try:
            ...
            os.chdir(old_cwd)
        assert result.exit_code == 0, f"init failed: {result.output}"
        # Skills mode should show /sp-plan (hyphenated)
        assert "/sp-plan" in result.output, (
            f"Expected /sp-plan in next steps but got:\n{result.output}"
        )
        # Must NOT show the dotted /sp.plan form
        assert "/sp.plan" not in result.output, (
            f"Should not show /sp.plan in skills mode:\n{result.output}"
        )
