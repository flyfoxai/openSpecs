...
The command templates keep `scripts:` frontmatter for both variants so the installer/rendering pipeline can select the correct platform command and replace `{SCRIPT}` in the command body. Do not treat `scripts:` as a universal host-side auto-execution hook: most installed host files receive already-rendered command text and no longer retain the raw `scripts:` metadata.
...
- run the host-rendered `sp.specify`, `sp.plan`, `sp.analyze`, and the rest of the active path
