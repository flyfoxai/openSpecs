...
- `sp.<command>` literals in memory and feature scaffolds are workflow concept names: command identity, layer membership, phase boundary, command duty descriptions ("X refreshes Y"), and time anchors (`before`, `after`, `during`, `ends at`).
- Only use the `__SPECKIT_COMMAND_<COMMAND>__` placeholder (replace `<COMMAND>` with the upper-case command stem) when the sentence is an executable hint that the model should run next: a "suggest" line, recovery hint, handoff `agent` field, or an owner / action column that the model would treat as a slash command to invoke.
- Do not replace `sp.<command>` in titles, layer lists, exclusion clauses, refresh-rule clauses, or `before / after / during sp.<command>` time anchors. The placeholder is rendered as `/sp.<command>` on dot hosts and `/sp-<command>` on skills hosts, which would pollute concept text with slash-call syntax.
- When in doubt, default to keeping the `sp.<command>` literal. Only switch to the placeholder when the surrounding sentence asks the reader (or model) to perform the next step.
