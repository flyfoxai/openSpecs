...
handoffs:
  - label: Create Tasks
    agent: __SPECKIT_COMMAND_TASKS__
...
3. Produce or refresh the delivery design outputs.
   - Use `.specify/templates/feature/delivery/*` and `.specify/templates/feature/memory/worksets/*` as structural references when creating or repairing delivery and workset artifacts.
...
5. Update agent-facing context if supported by the host.
   ...
   - Do not call host context refresh scripts unless they are actually installed in the target project.
...
- Suggest `__SPECKIT_COMMAND_TASKS__`.
