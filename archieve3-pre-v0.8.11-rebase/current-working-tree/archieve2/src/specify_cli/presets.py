...
class PresetManager:
    ...
    def _register_commands(
        ...
        for cmd in command_templates:
            ...
            if len(parts) >= 3 and parts[0] in {"sp", LEGACY_COMMAND_NAMESPACE}:
                ext_id = parts[1]
    ...
    def _skill_names_for_command(cmd_name: str) -> tuple[str, str]:
        ...
        short_name = strip_command_namespace(cmd_name)
        return canonical_skill_name(cmd_name), f"{LEGACY_COMMAND_NAMESPACE}.{short_name}"

    @staticmethod
    def _skill_title_from_command(cmd_name: str) -> str:
        """Return a human-friendly title for a skill command name."""
        return strip_command_namespace(cmd_name).replace(".", " ").replace("-", " ").title()
    ...
    def _register_skills(
        ...
        for cmd_tmpl in filtered:
            ...
            short_name = strip_command_namespace(cmd_name).replace(".", "-")
    ...
    def _unregister_skills(self, skill_names: List[str], preset_dir: Path) -> None:
        ...
        for skill_name in skill_names:
            ...
            short_name = strip_skill_prefix(skill_name)
