...
from .naming import (
    ...
    has_known_command_namespace,
    strip_command_namespace,
)
...
EXTENSION_COMMAND_NAME_PATTERN = re.compile(
    rf"^({CANONICAL_COMMAND_NAMESPACE}|{LEGACY_COMMAND_NAMESPACE})\.([a-z0-9-]+)\.([a-z0-9-]+)$"
)
...
class ExtensionManager:
    ...
    def _collect_manifest_command_names(manifest: ExtensionManifest) -> Dict[str, str]:
        ...
        - primary commands must use the canonical `sp.{extension}.{command}` shape
    ...
    def _register_extension_skills(
        ...
        for cmd_info in manifest.commands:
            ...
            # Derive skill name from command name using the canonical sp-* convention.
            skill_name = canonical_skill_name(cmd_name)
            ...
            title_name = strip_command_namespace(cmd_name).replace(".", " ").replace("-", " ").title()
...
class _AgentConfigProxy:
    """Live mapping proxy for the shared agent registrar config table."""

    @staticmethod
    def _configs():
        from .agents import CommandRegistrar as _AgentRegistrar

        _AgentRegistrar._ensure_configs()
        return _AgentRegistrar.AGENT_CONFIGS
...
class CommandRegistrar:
    ...
    AGENT_CONFIGS = _AgentConfigProxy()
...
class HookExecutor:
    ...
    def _skill_name_from_command(command: Any) -> str:
        ...
        if not has_known_command_namespace(command_id):
            return ""
        return canonical_skill_name(command_id)
