...
from .naming import canonical_command_id, canonical_skill_name
...
def _install_shared_infra(
    ...
    core = _locate_core_pack()
    try:
        manifest = IntegrationManifest.load("sp", project_path)
        manifest.version = get_speckit_version()
    except (FileNotFoundError, ValueError):
        manifest = IntegrationManifest("sp", project_path, version=get_speckit_version())

    def merge_tree(src_root: Path, dst_root: Path, *, should_copy: Callable[[Path], bool] | None = None) -> None:
    ...
    # Project assets copied into the target project root.
    ...
    # Shared page templates (not command templates, not project assets, not vscode-settings.json)
    ...
    if templates_src.is_dir():
        ...
        def should_copy_template(rel_path: Path) -> bool:
            ...
            if rel_path.parts[0] in {"commands", "project"}:
                return False
            # Prefer project-scoped template overrides when both sources define
            # the same .specify/templates/<file> path.
...
SKILL_DESCRIPTIONS = {
    ...
    "flow": "Map feature workflows, states, and cross-role transitions into explicit flow artifacts.",
    "ui": "Define screen structures, interactions, and UI coverage for the active feature area.",
    "gate": "Evaluate whether the current document set is ready to advance to the next delivery stage.",
    "bundle": "Assemble downstream delivery documents from approved specification artifacts.",
...
def init(
    ...
    def _display_cmd(name: str) -> str:
        if codex_skill_mode or agy_skill_mode or trae_skill_mode:
            return f"${canonical_skill_name(name)}"
        if claude_skill_mode:
            return f"/{canonical_skill_name(name)}"
        if kimi_skill_mode:
            return f"/skill:{canonical_skill_name(name)}"
        if cursor_agent_skill_mode:
            return f"/{canonical_skill_name(name)}"
        return f"/{canonical_command_id(name)}"
