...
- Uses a hyphenated frontmatter `name` value (e.g., `sp-foo-bar`) for shell compatibility, especially with ZSH
"""
...
from ...naming import canonical_skill_name
...
def format_forge_command_name(cmd_name: str) -> str:
    ...
    Examples:
        ...
        >>> format_forge_command_name("sp-plan")
        'sp-plan'
        >>> format_forge_command_name("sp.my-extension.example")
        'sp-my-extension-example'
        >>> format_forge_command_name("sp-my-extension-example")
        'sp-my-extension-example'
        >>> format_forge_command_name("sp.jira.sync-status")
        'sp-jira-sync-status'
    
    Args:
        cmd_name: Command name in dot notation (sp.foo.bar),
                  hyphenated format (sp-foo-bar), or plain name (foo)
    
    Returns:
        Hyphenated command name with 'sp-' prefix
    """
