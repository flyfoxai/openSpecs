...
class CommandRegistrar:
    ...
    def _ensure_configs(cls) -> None:
        if not cls._configs_loaded or not cls.AGENT_CONFIGS:
            try:
                configs = _build_agent_configs()
                if configs:
                    cls.AGENT_CONFIGS = configs
    ...
    def build_skill_frontmatter(
        ...
        if isinstance(original_frontmatter, dict):
            for key in ("scripts", "agent_scripts", "handoffs"):
                ...
                if value:
                    skill_frontmatter[key] = CommandRegistrar._resolve_frontmatter_placeholders(
    ...
    def _resolve_frontmatter_placeholders(value: Any, agent_name: str) -> Any:
        ...
        if isinstance(value, str):
            return CommandRegistrar.rewrite_project_relative_paths(
                value.replace("__AGENT__", agent_name)
            )
        if isinstance(value, list):
            return [CommandRegistrar._resolve_frontmatter_placeholders(item, agent_name) for item in value]
    ...
    def resolve_skill_placeholders(agent_name: str, frontmatter: dict, body: str, project_root: Path) -> str:
        """Resolve non-host placeholders for skills-backed agents.

        Skills-backed agents should preserve command metadata in frontmatter so
        the host can own script execution. We only replace placeholders that
        would otherwise remain syntactically invalid in the skill body.
        """
        try:
    ...
    def _compute_output_name(agent_name: str, cmd_name: str, agent_config: Dict[str, Any]) -> str:
        ...
        if agent_config["extension"] != "/SKILL.md":
            return canonical_command_id(cmd_name)

        return canonical_skill_name(cmd_name)
    ...
    def write_copilot_prompt(project_root: Path, cmd_name: str) -> None:
        ...
        canonical_name = canonical_command_id(cmd_name)
        prompt_file = prompts_dir / f"{canonical_name}.prompt.md"
