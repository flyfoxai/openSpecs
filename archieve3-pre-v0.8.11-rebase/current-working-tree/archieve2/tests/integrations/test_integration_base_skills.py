...
from tests.integrations.file_inventory import (
    COMMAND_STEMS,
...
class SkillsIntegrationTests:
    ...
    def test_skill_preserves_command_frontmatter_controls(self, tmp_path):
        """Scripts, agent scripts, and handoffs should survive skill rendering."""
        i = get_integration(self.KEY)
        m = IntegrationManifest(self.KEY, tmp_path)
        i.setup(tmp_path, m)

        plan_skill = i.skills_dest(tmp_path) / "sp-plan" / "SKILL.md"
        analyze_skill = i.skills_dest(tmp_path) / "sp-analyze" / "SKILL.md"

        plan_fm = yaml.safe_load(plan_skill.read_text(encoding="utf-8").split("---", 2)[1])
        analyze_fm = yaml.safe_load(analyze_skill.read_text(encoding="utf-8").split("---", 2)[1])

        assert "handoffs" in plan_fm
        assert isinstance(plan_fm["handoffs"], list)
        assert plan_fm["handoffs"][0]["agent"] == "sp.tasks"

        assert "scripts" in plan_fm
        assert "agent_scripts" in plan_fm
        assert ".specify/scripts/bash/" in plan_fm["scripts"]["sh"]
        assert ".specify/scripts/powershell/" in plan_fm["scripts"]["ps"]
        assert ".specify/scripts/bash/" in plan_fm["agent_scripts"]["sh"]
        assert ".specify/scripts/powershell/" in plan_fm["agent_scripts"]["ps"]
        assert "../../scripts/" not in plan_fm["scripts"]["sh"]
        assert "../../scripts/" not in plan_fm["agent_scripts"]["sh"]

        assert "scripts" in analyze_fm
        assert ".specify/scripts/bash/" in analyze_fm["scripts"]["sh"]
        assert ".specify/scripts/powershell/" in analyze_fm["scripts"]["ps"]
        assert "../../scripts/" not in analyze_fm["scripts"]["sh"]
    ...
    def test_pre_existing_skills_not_removed(self, tmp_path):
        """Pre-existing non-sp skills should be left untouched."""
        i = get_integration(self.KEY)
    ...
    def _expected_files(self, script_variant: str) -> list[str]:
        ...
        return sorted(
            skill_command_files(skills_prefix)
            + integration_support_files(self.KEY, script_variant)
        )
